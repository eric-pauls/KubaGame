...
A class named KubaGame for playing a board game called Kuba. See the rules [here](https://sites.google.com/site/boardandpieces/list-of-games/kuba).
Video resource for watching the game and its rules [here](https://www.youtube.com/watch?v=XglqkfzsXYc).
...
KubaGame class includes the following:

* An `init` method that takes as its parameters two tuples, each containing player name and color of the marble that the player is playing (ex: ('PlayerA', 'B'), ('PlayerB','W')) and it intializes the board. On the board R, B, W is used to represent Red, Black and White marbles. 
...
* A method called `get_marble` that takes the coordinates of a cell as a tuple and returns the marble that is present at the location. If no marble is present at the coordinate location return ' '.
...
The names of players are decided by the user of the game and will always be passed in the same way as it is passed when initializing the KubaGame object. The marbles are always represented as upper case R, B, W in the method calls and methods use the same representation when returning any relevant values. Similary, X, L, R, F and B that represent absence of marble and all the four directions respectively are also in upper case. 
...
Here's a very simple example of how the KubaGame class is used
